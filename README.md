...
You find the sample code for this guide at the [MessageBird Developer Guides GitHub repository](https://github.com/messagebirdguides/subscriptions-guide-go). You need to download or clone the repository to view the example code and follow along with the guide.
...
**IMPORTANT**: Make sure that all recipients in your subscriber list have been added to your contact list. If MessageBird fails to send an SMS message to any one recipient in the recipient list, the MessageBird API returns an error for the whole request but the SMS message is still sent to the remaining recipients.
